    ...
    >
      ...
      <div className="flex-grow min-w-0 flex flex-col justify-center">
        <div className="flex items-center gap-2 mb-1">
           <span className="font-black text-stone-800 text-lg leading-none truncate">{spell.name}</span>
           {isRitual && <span className="text-[10px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded font-bold border border-blue-200">仪式</span>}
           {isConcentration && <span className="text-[10px] bg-stone-200 text-stone-600 px-1.5 py-0.5 rounded font-bold border border-stone-300">专注</span>}
        </div>
   ...
   return (
      <div className="flex flex-col h-full overflow-hidden bg-stone-50">
         ...
         <div className="flex-grow overflow-y-auto p-4 grid grid-cols-1 md:grid-cols-2 gap-3">
            {spells.length === 0 ? (
               <div className="col-span-full text-center text-stone-500 py-12 italic text-lg">暂无匹配的法术数据。</div>
            ) : (
               spells.map(spell => (
                  <SpellListCard 
                     key={spell.id}
