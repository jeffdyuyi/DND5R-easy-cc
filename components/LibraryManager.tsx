...
import { ArrowLeft, FileUp, FileDown, RefreshCw } from 'lucide-react';
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          ...
          <div className="flex flex-col md:flex-row gap-4 items-end">
            <div className="flex gap-2">
              ...
              {onDeduplicate && (
                <button
                  onClick={() => { if (window.confirm('确定要进行数据清理吗？系统将尝试按名称合并重复项（优先保留官方版本）。')) onDeduplicate(); }}
                  className="bg-white border border-stone-300 text-stone-600 px-3 py-2 rounded-lg font-bold shadow-sm hover:bg-stone-50 transition-colors flex items-center gap-2 text-xs"
                  title="清理同名重复项"
                >
                  <RefreshCw className="w-4 h-4" /> 清理
                </button>
              )}
            </div>
