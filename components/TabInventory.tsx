...
const TabInventory: React.FC<Props> = ({ character, updateCharacter, libraryTools }) => {
  ...
  const PickerModal = ({ title, items, onSelect }: { title: string, items: ItemItem[], onSelect: (i: ItemItem) => void }) => (
  ...
  return (
    <div className="p-6 bg-white min-h-full font-sans text-stone-900">
      ...
      {/* Pickers */}
      {pickerType === 'weapon' && <PickerModal title="选择武器" items={WEAPON_DB} onSelect={(i) => addItem('inventoryWeapons', i)} />}
      {pickerType === 'armor' && <PickerModal title="选择护甲/盾牌" items={ARMOR_DB} onSelect={(i) => addItem('inventoryArmor', i)} />}
      {pickerType === 'gear' && <PickerModal title="选择杂物/法器" items={[...GEAR_DB.filter(i => i.type !== '弹药'), ...MAGIC_ITEM_DB]} onSelect={(i) => addItem('inventoryGear', i)} />}
      {pickerType === 'ammo' && <PickerModal title="选择弹药" items={GEAR_DB.filter(i => i.type === '弹药')} onSelect={(i) => addItem('inventoryGear', {...i, quantity: 20})} />}
      {pickerType === 'tool' && <PickerModal title="选择工具" items={TOOL_DB} onSelect={(i) => addItem('tools', i)} />}
