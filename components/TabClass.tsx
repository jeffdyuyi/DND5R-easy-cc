...
const TabClass: React.FC<Props> = ({ character, updateCharacter, libraryClasses, librarySubclasses, libraryFeats }) => {
           ...
           {canSelect && (
              <div className="mt-3 flex justify-end">
                 <button 
                    ...
                    className={`text-xs px-3 py-1.5 rounded flex items-center gap-1 transition-colors font-bold ${
                       selectedFeatData 
  ...
  const getDisplayFeatures = () => {
    ...
    // 2. Handle Subclass Logic
    if (character.subclass) {
      // If subclass is selected, remove the placeholder "Subclass Choice" feature from base class.
      baseFeatures = baseFeatures.filter(f => 
        !f.name.includes('子职') && !f.name.includes('Subclass')
      );

      const subData = subOptions.find(s => s.name === character.subclass);
      if (subData) {
        const subFeatures = subData.features.map(f => ({ ...f, sourceType: '子职' }));
        // Merge arrays
        baseFeatures = [...baseFeatures, ...subFeatures];
      }
    }
