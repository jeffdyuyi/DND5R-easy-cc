...
export const SpellEditor = ({ item, setItem }: { item: SpellItem, setItem: React.Dispatch<React.SetStateAction<SpellItem | null>> }) => {
   ...
   return (
      <div className="space-y-6">
         ...
         {/* Higher Level - Optional */}
         <div className="bg-amber-50 border border-amber-200 rounded-lg overflow-hidden">
            <label className="block text-sm font-bold text-amber-800 p-3 bg-amber-100 flex items-center justify-between">
               <span>升环施法效果</span>
               <span className="text-xs font-normal text-amber-600">(可选，留空则不显示)</span>
            </label>
            <textarea
               value={item.higherLevel || ''}
               onChange={e => setItem(p => p ? { ...p, higherLevel: e.target.value } : null)}
               className="w-full min-h-[5rem] p-4 border-0 bg-transparent focus:outline-none font-mono text-sm leading-relaxed"
